$(function () {
  $("#frmUpdateStudent").on(
    ...
    function (evt) {
      ...
      $("#frmUpdateStudent")
        .find('select[name="courseid"]')
        .find('option[data-departmentid="' + _this.val() + '"]')
        .removeClass("hidden");
  ...
  shutter.src = navigator.userAgent.match(/Firefox/)
    ? window.siteurl + "public/assets/webcamjs/shutter.ogg"
    : window.siteurl + "public/assets/webcamjs/shutter.mp3";

  $("#btnCapture").on("click", function () {
    $("#mainModal").modal("show");

    $("#mainModal .modal-body").html(
      '<div id="imageCapture"></div><div id="imageSnapshotPreview" class="hidden"></div>'
    );
    $("#mainModal .modal-content").prepend(
      '<div class="modal-header"><h4 class="modal-title">Snapshot</h4></div>'
    );
    $("#mainModal .modal-content").append(
      '<div class="modal-footer" style="text-align:center;"><button id="captureSnapshot" class="btn btn-info">Capture</button><button id="retrySnapshot" class="btn btn-info hidden">Retry</button><button id="okSnapshot" class="btn btn-info hidden">OK</button></div>'
    );

    Webcam.reset(); // Reset before attaching
    Webcam.set({
      width: 320,
      height: 240,
      image_format: "jpeg",
  ...
  $("#mainModal").on("click", "#okSnapshot", function (evt) {
    $("#frmUpdateStudent")
      ...
      .val($("#mainModal").find("#imageSnapshotPreview img").attr("src"));
    ...
    $("#frmUpdateStudent")
      .find(".imagePreview")
      .append(
        "<img style='margin: 0 auto;' class='img-responsive img-bordered' src='" +
          $("#mainModal").find("#imageSnapshotPreview img").attr("src") +
          "' />"
      );
