$(function () {
  ...
  $(window).on("keypress", function (e) {
    ...
    if (pressed == false) {
      setTimeout(function () {
        if (chars.length >= 8) {
          $("#previewArea").html(
            '<div class="box box-solid"><div class="box-body"><p class="text-center"><i class="fa fa-refresh fa-spin"></i></p></div></div>'
          );
          ...
          $("#txtBarcode").focus();
          $.post(
            $("#txtBarcode").parent().parent().attr("action"),
            $("#txtBarcode").parent().parent().serialize(),
            function (response) {
              $("#previewArea").html(response);
              var studentId = $("#previewArea")
  ...
  function loadEventAttendance(studentId = 0) {
    ...
    var eventId =
      $("input[name='eventid']").length == 1
        ? $("input[name='eventid']").val()
        : $("select[name='eventid']").val();
    $.get(
      window.siteurl +
        "attendance/loadEventAttendance/" +
        eventId +
        "/" +
        studentId,
      function (result) {
